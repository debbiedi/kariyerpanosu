...
export default function CareerCommandCenterV9() {
  ...
  return (
    <div className="flex w-full h-screen bg-slate-950 text-slate-200 font-sans overflow-hidden flex-col lg:flex-row relative">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900/95 backdrop-blur-xl border-r border-white/10 transition-transform duration-300 lg:relative lg:translate-x-0 shrink-0 flex flex-col shadow-2xl h-screen lg:h-full ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b border-white/10 flex justify-between items-center bg-gradient-to-r from-slate-900 to-slate-800 shrink-0">
          <div className="flex items-center gap-2 text-cyan-400 font-bold tracking-wider">
            ...
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500 text-sm">KARİYER-V9</span>
          </div>
          <button onClick={() => setMobileMenuOpen(false)} className="lg:hidden text-slate-400 p-1 rounded hover:bg-white/5">
            <X size={20} />
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full z-10 relative">
        ...
        <header className="h-16 border-b border-white/10 bg-slate-900/50 backdrop-blur-md flex items-center justify-between px-4 sticky top-0 z-30 shrink-0">
          <div className="flex items-center gap-3 flex-1">
            <button onClick={() => setMobileMenuOpen(true)} className="lg:hidden text-slate-400 p-1 rounded hover:bg-white/5">
              <Menu size={24} />
            </button>
        ...
        <main className="flex-1 flex overflow-hidden relative">
          ...
          <div className={`${selectedCountry ? 'hidden lg:flex' : 'flex'} flex-col flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 scroll-smooth min-w-0 border-r border-white/5 pb-20 lg:pb-6`}>
          ...
          {selectedCountry && (
            <div className="w-full lg:w-[480px] bg-slate-900/95 backdrop-blur-xl flex flex-col transition-all duration-300 shrink-0 shadow-[-10px_0_30px_rgba(0,0,0,0.5)] h-full">
              
              <div className="relative h-40 sm:h-48 shrink-0 overflow-hidden group">
                ...
                <button onClick={() => setSelectedCountry(null)} className="absolute top-6 left-6 z-20 lg:hidden bg-black/50 p-2 rounded-full text-white backdrop-blur-md">
                  <ArrowRight size={20} className="rotate-180" />
                </button>

                <div className="absolute bottom-4 sm:bottom-6 left-4 sm:left-8 z-20 right-4 sm:right-8">
                  <div className="flex items-center gap-3 mb-2">
                      <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${selectedCountry.tier === 'Tier 1' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : selectedCountry.tier === 'Tier 2' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'}`}>
                          {selectedCountry.tier}
                      </div>
                      <div className="h-px flex-1 bg-gradient-to-r from-white/20 to-transparent"></div>
                  </div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2 sm:mb-4 tracking-tight drop-shadow-lg">{selectedCountry.name}</h2>
                  
                  <div className="flex p-1 bg-slate-950/50 backdrop-blur-md rounded-xl border border-white/10">
                    <button onClick={() => setViewMode('career')} className={`flex-1 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-1 sm:gap-2 transition-all duration-300 ${viewMode === 'career' ? 'bg-gradient-to-r from-cyan-600 to-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                      <Briefcase size={12} /> <span className="hidden sm:inline">Prof.</span>
                    </button>
                    <button onClick={() => setViewMode('education')} className={`flex-1 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-1 sm:gap-2 transition-all duration-300 ${viewMode === 'education' ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                      <GraduationCap size={12} /> <span className="hidden sm:inline">Akad.</span>
                    </button>
                  </div>
                </div>
              </div>

              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6 bg-slate-900/50 pb-20 min-h-0">
                ...
                {viewMode === 'career' ? (
                  <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5 relative overflow-hidden">
                        ...
                        <div className="text-[10px] text-slate-500 mb-1 font-bold uppercase">Maaş</div>
                        <div className="text-sm sm:text-base font-bold text-emerald-400 truncate">{selectedCountry.salary}</div>
                      </div>
                      <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5 relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-2 opacity-10"><BarChart3 size={32}/></div>
                        <div className="text-[10px] text-slate-500 mb-1 font-bold uppercase">Zorluk</div>
                        <div className={`text-sm sm:text-base font-bold ${selectedCountry.difficulty > 50 ? 'text-red-400' : 'text-yellow-400'}`}>
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2 tracking-wider"><Layout size={12}/> Durum</h4>
                    ...
                    <div className="border-t border-white/10 pt-4 sm:pt-6">
                       <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-3 flex items-center gap-2 tracking-wider"><Settings size={12}/> Pozisyon</h4>
                       ...
                       <button onClick={() => performGoogleSearch(selectedCountry)} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white py-3 px-4 rounded-xl text-xs font-bold transition-all transform hover:scale-[1.02] flex items-center justify-center gap-2 shadow-xl shadow-blue-900/20 group border border-white/10">
                          <Search size={12} />
                  ...
                  <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="space-y-3">
                      <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5 flex justify-between items-center gap-2">
                        <div className="flex items-center gap-2 flex-1">
                            <div className="p-1.5 bg-purple-500/20 rounded-lg text-purple-400 shrink-0"><Coins size={14}/></div>
                            <span className="text-[10px] font-bold text-slate-400 uppercase">Ücret</span>
                        </div>
                        <div className="text-xs font-bold text-white text-right truncate">{selectedCountry.education.tuition}</div>
                      </div>
                      <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5 flex justify-between items-center gap-2">
                        <div className="flex items-center gap-2 flex-1">
                            <div className="p-1.5 bg-pink-500/20 rounded-lg text-pink-400 shrink-0"><Clock size={14}/></div>
                            <span className="text-[10px] font-bold text-slate-400 uppercase">Çalışma</span>
                        </div>
                        <div className="text-xs font-bold text-yellow-400 text-right truncate">{selectedCountry.education.workRights}</div>
                      </div>
                    </div>

                    <div>
                      <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-2 flex items-center gap-2"><Building size={12}/> Okullar</h4>
                    ...
                    <div className="bg-gradient-to-r from-purple-900/20 to-pink-900/20 p-4 rounded-xl border border-purple-500/20">
                      <h4 className="text-[10px] font-bold text-purple-400 uppercase mb-1 flex items-center gap-2"><Award size={12}/> Sonrası</h4>
                      <p className="text-sm text-white font-bold mb-1">{selectedCountry.education.postGrad}</p>
                ...
                <div className="pt-4 sm:pt-6 mt-4 border-t border-white/10">
                  ...
                  {showNoteInput ? (
                    ...
                    <div onClick={() => setShowNoteInput(true)} className="bg-slate-800/30 hover:bg-slate-800/60 border border-dashed border-slate-600 hover:border-cyan-500/50 rounded-xl p-3 text-xs text-slate-400 cursor-pointer transition-all flex flex-col items-center justify-center text-center gap-1 min-h-[60px] group">
                      {userNotes[selectedCountry.id] ? (
                          <p className="text-left w-full text-slate-300 whitespace-pre-wrap text-xs">{userNotes[selectedCountry.id]}</p>
                      ) : (
                          <>
                  ...
                  <a href={selectedCountry.link} target="_blank" rel="noreferrer" className="mt-4 flex items-center justify-center gap-2 w-full bg-white hover:bg-slate-200 text-slate-900 py-2.5 rounded-xl font-bold text-xs transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
                    Resmi Kaynak <ExternalLink size={12} />
                  </a>
                </div>
              </div>
            </div>
          ...
          {!selectedCountry && (
             <div className="lg:hidden flex flex-col items-center justify-center flex-1 bg-slate-900/50 border-l border-white/10 text-slate-600 relative z-0">
               <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(6,182,212,0.05),transparent)]"></div>
               <Globe size={60} className="mb-4 opacity-20 animate-pulse" />
               <p className="text-xs sm:text-sm font-medium tracking-wide text-center px-4">BİR HEDEF SEÇİN</p>
             </div>
          )}
        </main>
      </div>
